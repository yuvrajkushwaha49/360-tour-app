...
ipcMain.handle('dialog:open-directory', async (event, projectDir?, dirKey?, locationName?) => {
...
ipcMain.handle('stitch:run', async (event, config) => {
  return new Promise((resolve, reject) => {
    ...
    const pythonExe = path.join(app.getAppPath(), 'python_engine/.venv/Scripts/python.exe');
    const scriptPath = path.join(app.getAppPath(), 'python_engine/stitcher.py');

    const pyProcess = spawn(pythonExe, [scriptPath, configPath]);
